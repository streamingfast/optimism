...
use reth_chain_state::{DeferredTrieData, ExecutedBlock, ExecutionTimingStats};
    ...
    Evm: ConfigureEvm<Primitives = N> + 'static,
{
    ...
    where
        ...
        Evm: ConfigureEngineEvm<T::ExecutionData, Primitives = N>,
        Evm::BlockExecutorFactory: alloy_evm::block::BlockExecutorFactory<
            ...
            + SignatureFields,
    {
        ...
        // multiproofs).  The overlay manager carries in-memory block state so workers can resolve
        // blocks that haven't been persisted yet.
        ...
        let (output, senders, receipt_root_rx) = match fh_tracer.as_mut() {
            Some(tracer) => match self.execute_and_trace_block(
                state_provider,
                env,
                &input,
                tracer,
            ...
            StateRootStrategy::StateRootTask => {
                ...
                match task_result {
                    Ok(StateRootComputeOutcome {
                        ...
                        // NOTE: the upstream debug helper `compare_trie_updates_with_serial` is
                        // omitted here because it depends on the `super::trie_updates` module
                        // which is crate-private to `reth_engine_tree`. The
                        // `always_compare_trie_updates` flag is intentionally a no-op when running
                        // through the Firehose engine validator — re-enable in SF reth upstream
                        // and re-export `trie_updates::compare_trie_updates` if this becomes
                        // necessary for debugging.
                        let _ = (provider_factory, overlay_builder);
    ...
    /// Mirrors `BasicEngineValidator::overlay_builder_for_parent` exactly.
...
where
    ...
    EV::Validator: reth_engine_primitives::PayloadValidator<
        ...
        > + Clone,
    TxTy<<Node::Types as reth_node_api::NodeTypes>::Primitives>: alloy_consensus::Transaction
        + alloy_consensus::transaction::TxHashRef
        + SignatureFields,
{
    type EngineValidator =
        OpFirehoseEngineValidator<Node::Provider, Node::Evm, EV::Validator>;

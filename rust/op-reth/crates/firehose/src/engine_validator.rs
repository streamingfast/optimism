//! OP Stack-specific engine validator that mirrors [`reth_engine_tree::tree::OpFirehoseEngineValidator`]
//! and adds the Firehose live-tracing path on the engine-API flow.
//!
...
use reth_chain_state::{
    ExecutedBlock, ExecutionTimingStats, StateTrieOverlayManager,
};
...
use reth_engine_tree::tree::state_root_strategy::{
    PayloadStateRootHandle, StateRootHintStream, StateRootUpdateStream,
};
...
use tracing::{debug, debug_span, error, instrument, trace, warn, Span};

use alloy_op_evm::OpEvmFactory;
use reth_firehose::{
    is_tracer_initialized, mapper::SignatureFields, FirehoseBlockTracer, FirehoseWrappedExecutor,
};
    ...
    Evm: ConfigureEvm<Primitives = N> + 'static,
{
            ...
            + SignatureFields,
    {
        ...
        let convert_to_block = match &input {
            BlockOrPayload::Payload(_) => {
                ...
                let handle = self
                    .runtime
                    .spawn_blocking_named("payload-convert", move || {
        ...
        let convert_to_block =
            move |input: BlockOrPayload<T>| -> Result<SealedBlock<N::Block>, NewPayloadError> {
                match convert_to_block {
                    ...
                    Either::Right(()) => {
                        let BlockOrPayload::Block(block) = input else {
                            unreachable!()
                        };
                        Ok(block)
                    }
        ...
        Evm: ConfigureEngineEvm<T::ExecutionData, Primitives = N>,
    {
        ...
        if !self.config.precompile_cache_disabled() {
            ...
            executor
                .evm_mut()
                .precompiles_mut()
                .map_cacheable_precompiles(|address, precompile| {
                    let metrics = self
        ...
        self.metrics
            .record_post_execution(post_exec_start.elapsed());
        ...
        self.metrics
            .record_block_execution(&output, execution_duration);
        self.metrics
            .record_block_execution_gas_bucket(output.result.gas_used, execution_duration);
        ...
        Err: core::error::Error + Send + Sync + 'static,
    {
        ...
        loop {
            ...
            self.metrics
                .record_transaction_execution(tx_start.elapsed());
        ...
        V: PayloadValidator<T, Block = N::Block>,
    {
        ...
        if let Err(err) =
            self.consensus
                .validate_block_post_execution(block, output, receipt_root_bloom, None)
        {
            // call post-block hook
            self.on_invalid_block(parent_block, block, output, None, ctx.state_mut());
            return Err(err.into());
    ...
    fn on_invalid_block(
        ...
        self.invalid_block_hook
            .on_invalid_block(parent_header, block, output, trie_updates);
    }
    ...
    fn calculate_timing_stats(
        ...
        let accounts_deleted = output
            ...
            .state
            .values()
            .filter(|acc| acc.was_destroyed())
            .count();
        let storage_slots_changed = output
            .state
            .state
            .values()
        ...
        let eip7702_delegations_set = output
            .state
            .contracts
            .values()
            .filter(|bytecode| bytecode.is_eip7702())
            .count();
        ...
        let (storage_cache_hits, storage_cache_misses) = cache_stats
            ...
            .unwrap_or_default();
        let (code_cache_hits, code_cache_misses) = cache_stats
            .as_ref()
            .map(|s| (s.code_hits(), s.code_misses()))
            .unwrap_or_default();
        ...
        alloy_consensus::Transaction + alloy_consensus::transaction::TxHashRef + SignatureFields,
{
    ...
    fn validate_payload(
        ...
        self.validate_block_with_state(BlockOrPayload::Payload(payload), ctx)
            .map(|(executed_block, execution_timing_stats)| {
    ...
    Evm: ConfigureEvm,
{
    fn wait_for_caches(&self) -> CacheWaitDurations {
        ...
        self.runtime
            .spawn_blocking_named("wait-sparse-tri", move || {
                let _ =
                    sparse_trie_tx.send(state_trie_overlays.wait_for_sparse_trie_availability());
            });

        let sparse_trie = sparse_trie_rx
            .recv()

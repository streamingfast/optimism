...
* Bumped `streamingfast/reth` to `world-chain-v2.4.3-fh3.1-2`, which stops recording keccak
  preimages larger than 256 bytes. `Call.keccak_preimages` exists so a consumer can walk a storage
  slot back to the expression that produced it, and Solidity's slot derivations are small: 32 bytes
  for a dynamic array or a `bytes`/`string` longer than 31 bytes, 64 bytes for a mapping with a
